...
mnist_train_label = mnist_train_label.astype(np.float32)
mnist_test_label = mnist_test_label.astype(np.float32)
...
x = tf.placeholder(tf.float32, shape=[None, 45,45])
batch_images = tf.reshape(x, [-1, 45, 45, 1])
...
cost = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(labels=tf.cast(label, dtype=tf.int32), logits=logits))
...
correct_prediction = tf.equal(tf.argmax(logits, 1), tf.cast(label, dtype=tf.int64))
